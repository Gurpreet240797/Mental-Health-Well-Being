...
export default function RegistrationForm({ setSession }) {
    ...
    async function handleSubmit(event) {
        ...
        await fetch("http://localhost:8080/home/register", request)
            .then(response => {
                if (response.status === 201) {
                    setSubmissionStatus(1);
                    submitLogin({ email: state.email, password: state.password }).then(s => setSession(s));
                                        ...
                                        <Form onSubmit={handleSubmit}>
                                            ...
                                            {submissionStatus !== 0 && alertState.show &&
                                                <Alert variant={submissionStatus === 1 ? "success" : (submissionStatus === 2 ? "warning" : "danger")} onClose={() => setAlertState({ show: false, message: "" })} dismissible>
                                                    ...
                                                    {submissionStatus === 1 &&
                                                        <p> You can now log in by clicking <Link to="/login">here</Link>.</p>
                                                    }
                                                </Alert>
                                            }
                                            ...
                                            <Form.Group className="mb-3" controlId="firstNameInput">
                                                ...
                                                <Form.Control type="text" value={state.firstName} name="firstName" onChange={handleChange} placeholder="Enter your first name here" />
                                            </Form.Group>

                                            <Form.Group className="mb-3" controlId="lastNameInput">
                                                <Form.Label>Last Name*</Form.Label>
                                                <Form.Control type="text" value={state.lastName} name="lastName" onChange={handleChange} placeholder="Enter your last name here" />
                                            </Form.Group>

                                            <Form.Group className="mb-3" controlId="dateOfBirthInput">
                                                <Form.Label>Birthdate*</Form.Label>
                                                <Form.Control type="date" value={state.dateOfBirth} name="dateOfBirth" onChange={handleChange} placeholder="Please select your birthdate" />
                                            </Form.Group>

                                            <Form.Group className="mb-3" controlId="addressInput">
                                                <Form.Label>Address*</Form.Label>
                                                <Form.Control type="text" value={state.address} name="address" onChange={handleChange} placeholder="Please enter your address here" />
                                            </Form.Group>

                                            <Form.Group className="mb-3" controlId="phoneNumberInput" pattern="^-?[0-9]\d*\.?\d*$">
                                                <Form.Label>Phone Number*</Form.Label>
                                                <Form.Control type="tel" value={state.phoneNumber} name="phoneNumber" onChange={handlePhoneNumberInput} placeholder="Please enter your phone number here" />
                                            </Form.Group>

                                            <Form.Group className="mb-3" controlId="emailInput">
                                                <Form.Label>Email*</Form.Label>
                                                <Form.Control type="email" value={state.email} name="email" onChange={handleChange} placeholder="Please enter your phone number here" />
                                            </Form.Group>
                                            ...
                                            {state.role !== "2" &&
                                                <Form.Group className="mb-3" controlId="registrationNumberInput">
                                                    ...
                                                    <Form.Control type="number" value={state.registrationNumber} name="registrationNumber" onChange={handleChange} placeholder="Please enter your registration number here" />
                                                </Form.Group>
                                            }

                                            <Button variant="primary" type="submit" disabled={submissionDisabled}>Register</Button>
                                        </Form>

                                        <hr />
                                        <div className="auth-option text-center pt-2">Already Have An Account? <Link className="text-link" to="/login" >Log In</Link></div>
